 $(document).ready(function() {
    ...
    const RTP_CONFIG = {
        ...
        "NORMAL": { scatterChance: 0.04, goldChance: 0.45, baseWinChance: 0.60, maxCascades: 8, weights: { high: 20, mid: 30, low: 40 } },
    ...
    const SFX = {
        drop: function() {
            ...
            osc.type = 'triangle'; osc.frequency.setValueAtTime(700, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(350, audioCtx.currentTime + 0.04);
            gain.gain.setValueAtTime(0.18, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + 0.06);
            osc.connect(gain); gain.connect(masterGain); osc.start(); osc.stop(audioCtx.currentTime + 0.06);
        },
        spin: function() {
            if (!audioCtx) return;
            const noise = audioCtx.createBufferSource(); const buffer = audioCtx.createBuffer(1, audioCtx.sampleRate * 0.6, audioCtx.sampleRate);
            const data = buffer.getChannelData(0);
            for (let i = 0; i < buffer.length; i++) data[i] = (Math.random() * 2 - 1) * (1 - i / buffer.length) * 0.4;
            noise.buffer = buffer;
            const filter = audioCtx.createBiquadFilter(); filter.type = 'bandpass'; filter.frequency.value = 1000; filter.Q.value = 0.5;
            noise.connect(filter); filter.connect(masterGain); noise.start();
        },
        reelStop: function() {
            if (!audioCtx) return;
            const osc = audioCtx.createOscillator(); const gain = audioCtx.createGain();
            osc.type = 'sine'; osc.frequency.setValueAtTime(300, audioCtx.currentTime); osc.frequency.exponentialRampToValueAtTime(600, audioCtx.currentTime + 0.05);
            gain.gain.setValueAtTime(0.25, audioCtx.currentTime); gain.gain.exponentialRampToValueAtTime(0.001, audioCtx.currentTime + 0.08);
            osc.connect(gain); gain.connect(masterGain); osc.start(); osc.stop(audioCtx.currentTime + 0.08);
        },
    ...
    // ==========================================
    // GRID FUNCTIONS - INSTANT DROP SYSTEM
    // ==========================================
    function getReelHeight(i){return(i===0||i===4)?4:5;}
    
    // FIX: Tambah parameter style string
    function createTileHtml(a,g,s,d,extraStyle){
        ...
        // extraStyle untuk inject opacity:0
   ...
   async function animateFallDown(){
        for(let ri=0; ri<5; ri++){
            let rl=$(`#reel${ri}`);
            // Ambil semua tile yang bukan sedang meledak
            ...
            ts.each(function(){
                ...
                // Jika jarak cukup jauh, lakukan animasi
                if(fd > 3){
                    // Set state awal: di atas (translateY negatif) & invisible
                    // Kita gunakan nilai fd yang sudah dihitung
                    t.css({
                    ...
                    // FIX: Jika sudah pas (fd kecil), PAKSA tampilkan
                    // Ini mengatasi bug "kotak kosong" jika posisi sudah benar
                    t.css({
                        'transform': 'translateY(0)',
                        'opacity': 1
                    });
    ...
    async function cascadeLogic(){
        ...
        if(res.payout>0){
            ...
            res.tiles.forEach(el=>{
                ...
                if(isG){
                    SFX.breakGold();spawnParticles(cx,cy,'explode','gold');
                    $(el).removeClass('gold winning-glow').addClass('wild wild-transform').attr('data-type','W').find('img').attr('src',wildImg);
                    goldTiles.push(el);
                }
                else{
                    SFX.breakTile();spawnParticles(cx,cy,'explode',tr);
            ...
            // FIX: Isi dengan tile INVISIBLE dulu (style opacity:0)
            fillGridCascade(); 
   ...
   function fillGridCascade(){
        for (let reelIdx = 0; reelIdx < 5; reelIdx++) {
            ...
            for (let i = 0; i < missingCount; i++) {
                ...
                // FIX: Set opacity 0 DAN translateY negatif agar selalu "di atas"
                // Ini memaksa animasi jatuh selalu berjalan
    ...
    $("#btn-confirm-topup").on('click', function() {
        ...
        $("#topup-success-amount").text(formatRupiah(selectedTopupAmount)); $("#topup-success-balance").text(formatRupiah(balance));
        $("#topup-success-time").text(new Date().toLocaleString('id-ID'));
        $("#topup-success-modal").css("display", "flex"); selectedTopupAmount = 0;
    });
    ...
    $("#menu-withdraw").on('click', function() {
        ...
        $("#name-field").hide(); $("#account-label").text("Nomor Tujuan");
        $("#withdraw-modal").css("display", "flex");
    });
    $("#withdraw-method").on('change', function() {
        const m = $(this).val();
        if (['BCA','MANDIRI','BNI','BRI'].includes(m)) { $("#name-field").slideDown(200); $("#account-label").text("No. Rekening"); }
        else if (['DANA','OVO','GOPAY','SHOPEEPAY'].includes(m)) { $("#name-field").slideUp(200); $("#account-label").text("Nomor HP"); }
        else { $("#name-field").slideUp(200); $("#account-label").text("Nomor Tujuan"); }
    });
    ...
    $("#btn-confirm-withdraw").on('click', function() {
        let amt = parseInt($("#withdraw-amount").val())||0, met = $("#withdraw-method").val(), acc = $("#withdraw-account").val().trim(), nm = $("#withdraw-name").val().trim();
        ...
        $("#success-amount").text(formatRupiah(amt)); $("#success-method").text(met); $("#success-account").text(['BCA','MANDIRI','BNI','BRI'].includes(met) ? acc+" ("+nm+")" : acc);
        $("#withdraw-modal").css("display", "none"); $("#withdraw-success-modal").css("display", "flex"); SFX.win();
    });
