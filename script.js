 $(document).ready(function() {
    ...
    function initAudio() {
        ...
        createBgParticles(); // Start BG animation
    }
    ...
    const SFX = {
        ...
        reelStop: function() {
            ...
            osc.connect(gain); gain.connect(masterGain); osc.start(); osc.stop(audioCtx.currentTime + 0.15);
        },
    ...
    let stopOnBigWin = true; // Smart Auto
    ...
    // Klik Angka/Opsi -> Hanya pilih (Select), jangan langsung jalan
    $(".opt").on('click', function(){
        const val = $(this).data('val');
        $(".opt").removeClass("selected"); // Hapus selected dari semua
        $(this).addClass("selected"); // Tandai yang ini sebagai selected
        // Simpan value sementara, tapi jangan set autoRemaining dulu
        $(this).data('selected-val', val); 
    });

    // Klik Tombol MULAI -> Baru jalankan
    $("#btn-start-auto").on('click', function(){
        // Cek apakah ada yang dipilih? Kalau tidak, default 10
        let selectedOpt = $(".opt.selected");
        let val = selectedOpt.length > 0 ? selectedOpt.data('val') : 10; 
        
        autoRemaining = val === 1000 ? 999999 : val;
        $("#auto-modal").css("display", "none");
        updateUIData();
        processSpin(); // Langsung mulai
    });

    // Default: Pilih opsi pertama (10) saat modal dibuka (opsional biar user ga lupa pilih)
    $("#auto-btn").on('click', function(){
        // Buka modal
        if(isSpinning) return;
        if(freeSpinsLeft>0){ showAlert("LOCKED","Tidak dapat menggunakan Auto selama Free Spins!"); return; }
        
        // Reset pilihan ke default 10 saat buka
        $(".opt").removeClass("selected");
        $(".opt[data-val='10']").addClass("selected"); 
    ...
    function renderTopupHistory() {
        ...
        if (topupHistory.length === 0) {
            html = '<div class="history-empty">Belum ada history topup</div>';
        } else {
                ...
                html += `
                    <div class="history-item">
                        <div class="history-item-header">
                            ...
                            <span class="history-status status-success">${item.status}</span>
                        </div>
    ...
    function renderWithdrawHistory() {
        ...
        if (withdrawHistory.length === 0) {
            html = '<div class="history-empty">Belum ada history withdraw</div>';
        } else {
    ...
    $("#menu-history-topup").on('click', function() {
        $("#menu-popup").hide();
        renderTopupHistory();
        $("#history-topup-modal").css("display", "flex");
    });

    $("#menu-history-withdraw").on('click', function() {
        $("#menu-popup").hide();
        renderWithdrawHistory();
        $("#history-withdraw-modal").css("display", "flex");
    });
    ...
    // FIX: Logika perubahan Label dan Nama
    $("#withdraw-method").on('change', function() {
        const m = $(this).val();
        
        // Selalu tampilkan kolom nama untuk semua metode
        if (m) {
        ...
        if (['BCA','MANDIRI','BNI','BRI'].includes(m)) {
            ...
            $('#withdraw-account').attr('placeholder','Nomor HP');
        } else {
            $("#account-label").text("Nomor Tujuan");
            $('#withdraw-account').attr('placeholder','Nomor Tujuan');
        }
    });
    ...
    $("#btn-confirm-withdraw").on('click', function() {
        ...
        // FIX: Nama wajib diisi SEMUA metode
    ...
    // ==========================================
    // PAYTABLE (Updated to menu)
    // ==========================================
    $("#menu-paytable").on('click', function(){
        $("#menu-popup").hide();
        $("#paytable-modal").css("display", "flex");
    });
